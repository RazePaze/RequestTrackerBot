...
async def forwardedHandler(bot:Update, msg:Message):
    ...
    if forwardInfo.type == "channel":   # If message forwarded from channel
        await msg.reply_text(
            f"<b>Hey😁, Your Channel ID is <code>{forwardInfo.id}</code>\n\n😊Join @Toman_Network for getting more awesome 🤖bots like this.</b>",
            parse_mode = "html"
        )
    return
        ...
        try:
            ...
            for document in documents:
                ...
                try:
                    ...
                    await msg.reply_text(
                        "<b>😒Group ID is wrong.\n\n😊Join @Toman_Network :p for getting more awesome 🤖bots like this.</b>",
                        parse_mode = "html"
                    )
